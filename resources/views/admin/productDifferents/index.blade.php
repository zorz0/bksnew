...
@can('product_different_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.product-differents.create') }}">
                        ...
                        <tr data-entry-id="{{ $productDifferent->id }}">
                            ...
                            <td>
                                @can('product_different_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.product-differents.show', $productDifferent->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('product_different_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.product-differents.edit', $productDifferent->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('product_different_delete')
                                    <form action="{{ route('admin.product-differents.destroy', $productDifferent->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('product_different_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.product-differents.massDestroy') }}",
