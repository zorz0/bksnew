        ...
        <div class="col-md-12">
            @can('websit_info_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.websit-infos.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $websitInfo->id }}">
                                        ...
                                        <td>
                                            @can('websit_info_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.websit-infos.show', $websitInfo->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('websit_info_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.websit-infos.edit', $websitInfo->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('websit_info_delete')
                                                <form action="{{ route('frontend.websit-infos.destroy', $websitInfo->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('websit_info_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.websit-infos.massDestroy') }}",
